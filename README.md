...
Ces bases de données sont composées des variables suivantes (les liens fournis renvoient aux sources primaires des données collectées et assemblées dans les deux bases de données) :
   ...
   - IG: Proportion mensuelle d'articles de presse contenant la mobilisation contestataire (contre les mesures sanitaires) d'au moins un groupe d'intérêts touché par les mesures sanitaires (commerces, services, restaurants, services culturels) [issue d'un codage d'articles de presse de la base de données de l'Observatoire de la circulation de l'information](https://oci-ifo.org/en/home/).
...
These databases are composed of the following variables (the provided links hereafter refer to the primary sources of the data collected and assembled in these databases) :
   ...
   - IG: Monthly proportion of press articles containing the protest mobilization (against health measures) of at least one group of interests affected by the health measures (commerce, services, restaurants, cultural services) from a manual coding of [press articles from the database of the Observatory of Information Flow.](https://oci-ifo.org/en/home/).
